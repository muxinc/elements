...
describe('<mux-player>', () => {
  ...
  it("signing tokens generate correct asset URL's", async function () {
    ...
    waitUntil(() => !!muxVideo.getAttribute('src'), '<mux-video> src never set');
    assert.equal(
      muxVideo.getAttribute('src'),
    ...
    waitUntil(() => !!mediaPosterImage.getAttribute('src'), '<media-poster-image> src never set');
    assert.equal(
      mediaPosterImage.getAttribute('src'),
    ...
    waitUntil(() => !!storyboardTrack.getAttribute('src'), 'storyboard <track> src never set');
    assert.equal(
  ...
  describe('buffered behaviors', function () {
    ...
    (isSafari ? it.skip : it)('should have something in the buffer if canplay', async function () {
      ...
      waitUntil(() => playerEl.readyState >= 3, '<mux-player> readyState never reached canPlay');
