...
describe('<mux-player> template render', () => {
  ...
  it('default template without props', function () {
    ...
    assert.equal(
      ...
      normalizeAttributes(
        `<media-theme-base template="media-theme-mux" class="size-" default-showing-captions="" disabled="" nohotkeys="" layout="on-demand" exportparts="${exportParts}"><mux-video slot="media" crossorigin="" playsinline="" exportparts="video"></mux-video><mxp-dialog no-auto-hide=""><p></p></mxp-dialog></media-theme-base>`
      )
    );
  });
