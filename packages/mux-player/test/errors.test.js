...
describe('errors', () => {
  ...
  it('default message for MediaError.MEDIA_ERR_NETWORK', async function () {
  ...
  it('default message for MediaError.MEDIA_ERR_DECODE', async function () {
  ...
  it('default title for MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED', async function () {
  ...
  it('default title for any error', async function () {
    const error = new MediaError(undefined, 0);
    const { dialog } = await getErrorLogs(error);
    assert.equal(dialog.title, 'Error');
  });

  it('offline error logs', async function () {
    const error = new MediaError(undefined, MediaError.MEDIA_ERR_DECODE);
    const offline = true;
    const { dialog } = await getErrorLogs(error, offline);
  ...
  it('412 response code message for MediaError.MEDIA_ERR_NETWORK', async function () {
    const error = new MediaError(undefined, MediaError.MEDIA_ERR_NETWORK);
  ...
  it('404 response code message for MediaError.MEDIA_ERR_NETWORK', async function () {
    const error = new MediaError(undefined, MediaError.MEDIA_ERR_NETWORK);
  ...
  it('403 response code message for MediaError.MEDIA_ERR_NETWORK', async function () {
    const error = new MediaError(undefined, MediaError.MEDIA_ERR_NETWORK);
    error.data = { response: { code: 403 } };

    const { dialog, devlog } = await getErrorLogs(error);
  ...
  it('403 response code message w/ expired token for MediaError.MEDIA_ERR_NETWORK', async function () {
    ...
    const { dialog, devlog } = await getErrorLogs(error, false, playbackId, playbackToken);
  ...
  it('403 response code message w/ playback id mismatch for MediaError.MEDIA_ERR_NETWORK', async function () {
  ...
  it('403 response code message w/ token type mismatch for MediaError.MEDIA_ERR_NETWORK', async function () {
