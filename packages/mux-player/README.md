...
By default `<mux-player>` will try to use native playback via the underlying `<video>` tag whenever possible.
...
| `prefer-playback`             | `"mse" \| "native" \| undefined`                                    | Set the preference for the playback type; `mse` (currently hls.js), or `native` playback if it is supported (e.g. in Safari). For more, see the section on [`preferPlayback`](#preferPlayback)                                                                                                                                                                                                   | `undefined`   |
