...
| `upload-complete`       | `boolean`           | (Read-only) Toggles visual status of progress bar when an upload is completed. Can be targeted with CSS if you want to control styles when the upload completes i.e. `mux-uploader[upload-error]`.                                                                                       | `false`       |
...
| `use-large-file-workaround` | `boolean`           | Enables a less memory-efficient way of loading and chunking files for environments that don't reliably handle [`ReadableStream` for large files](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API/Using_readable_streams).      | `false`       |
