...
- `cap-level-to-player-size`: Controls whether hls.js caps video quality based on player dimensions. When unset (default), uses Mux's optimized behavior that caps quality to player size but maintains a 720p minimum floor. Set the attribute to explicitly enable standard hls.js capping (may drop below 720p). To disable capping entirely, set `capLevelToPlayerSize = false` via JavaScript.
