...
- `min-bandwidth-sample-duration-ms: number (ms)`: Overrides the hls.js EWMA bandwidth estimator's minimum sample-duration floor (`minDelayMs_`). Lowering it lets very short transfers (small segments on fast networks) contribute to the estimate; raising it better defends against cache-hit / timer-resolution noise. Set to `0` to disable the clamp entirely. Defaults to the upstream hls.js value (`50`).
