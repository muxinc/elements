...
- `max-resolution`: Limits the highest resolution rendition requested from the server. Renditions above this are excluded from the playlist entirely. For client-side dimension-based capping, see `cap-rendition-to-player-size`. When using signed URLs, include `max_resolution` in your signed token instead.
- `min-resolution`: Limits the lowest resolution rendition requested from the server. Renditions below this are excluded from the playlist entirely. When using signed URLs, include `min_resolution` in your signed token instead.
- `max-auto-resolution`: Caps automatic resolution selection to align with [Mux Video pricing tiers](https://www.mux.com/docs/pricing/video#resolution-based-pricing). The player won't automatically select resolutions above this cap. Unlike `max-resolution`, all renditions remain available. Unlike `cap-rendition-to-player-size`, this is independent of player dimensions. Accepts: `"720p"`, `"1080p"`, `"1440p"`, or `"2160p"`.
...
- `cap-rendition-to-player-size`: Caps video resolution based on the player's display dimensions to avoid downloading video larger than can be displayed. When unset (default), caps to player size with a 720p minimum floor. Set the attribute to cap strictly to player dimensions (may use lower resolutions for small players). To disable dimension-based capping entirely, set `capRenditionToPlayerSize = false` via JavaScript. Independent of `max-resolution` (server-side) and `max-auto-resolution` (pricing cap).
...
- `max-reconnect-retries: number`: Opt-in recovery from network interruptions that stall playback (off by default). When set to a positive number, after hls.js exhausts its own internal retries and reports a fatal connectivity error, the player waits (with exponential backoff) and tells hls.js to try again up to this many attempts and resumes automatically once the connection returns (it also fast-paths off the browser's `online` event), showing a transient "Reconnecting…" state meanwhile. Unset or `0` keeps the prior behavior: a fatal connectivity error surfaces immediately.
This is distinct from hls.js's own per-request retry config (it governs reconnecting _after_ hls.js has given up). Note: each attempt also waits out hls.js's full internal retry cycle, so larger values can mean a noticeably longer wait before the terminal error — pick a value accordingly.
