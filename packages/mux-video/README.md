...
- `max-auto-resolution`: Cap the default resolution selection based on total pixels to match Mux Video pricing tiers. Accepts: `"720p"`, `"1080p"`, `"1440p"`, or `"2160p"`. If there's an exact match, it will be used. Otherwise, selects the highest quality rendition that doesn't exceed the cap. Other values are ignored.
