...
export const loadMedia = (
  ...
  if (mediaEl && shouldUseNative) {
    ...
    // hls.js retries non-fatal network errors itself; once it gives up (fatal) it stops loading
    // and won't resume - including on flaky networks where the browser never fires `offline`/
    // `online` (navigator.onLine only reflects a network link, not whether requests succeed).
    //
    ...
    //
    // The retry loop and its "Reconnecting..." UI are tied to the stall (same condition as the
    // loading spinner): while there's still playable buffer, hls.js's own retry handles refilling
    // and we stay silent.
    //
    ...
    let retryCount = 0; // retries used in the current stall episode (drives backoff + the cap)
    let retrying = false; // the bounded backoff loop is active for the current stall
    let terminalShown = false; // retries exhausted: terminal error shown, polling stopped
    // Whether the manifest has loaded at least once. Before it has, recovery must re-request the
    // source (startLoad won't re-fetch a manifest that failed to load) - e.g. origin down at load.
    ...
    const startRecoveryIfStalled = () => {
      const state = muxMediaState.get(mediaEl);
      if (!state?.networkError || terminalShown || retrying) return;
