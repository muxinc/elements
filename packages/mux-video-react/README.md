...
- `capRenditionToPlayerSize: boolean`: Controls whether hls.js caps video quality based on player dimensions. When `undefined` (default), uses Mux's optimized behavior that caps quality to player size but maintains a 720p minimum floor. Set to `true` to explicitly enable standard hls.js capping (may drop below 720p). Set to `false` to disable capping entirely.
